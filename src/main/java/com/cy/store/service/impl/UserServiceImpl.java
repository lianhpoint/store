...
public class UserServiceImpl implements IUserService {
    ...
    public void reg(User user) {
        ...
        if(result!=null){
            //抛出异常
            throw new UsernameDupcalitedException("用户名被占用");
        }

        //密码加密处理的实现：md5算法（因为在数据库内密码是以明文的形式显示，为了数据安全，所以需要进行加密的处理）
    ...
    public User login(String username, String password) {
        ...
        //User user = userMapper.findByUsername(username);前面已经用过了，所以不用在重复的写这一句
        //之所以这么写，不是用整个user对象，是为了提升系统的性能，因为传输的数据量少了，而且暂时也用不到其它的数据
        User user = new User();
